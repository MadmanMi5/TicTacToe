...
def new_game():
    ...
    label.config(text="You are " + currentPlayer)
...
def check_winner():
    for buttonRow in range(3):
        if buttons[buttonRow][0]['text'] == buttons[buttonRow][1]['text'] == buttons[buttonRow][2]['text'] != "":
            buttons[buttonRow][0].config(bg=who_won(buttons[buttonRow][0]['text']))
            buttons[buttonRow][1].config(bg=who_won(buttons[buttonRow][1]['text']))
            buttons[buttonRow][2].config(bg=who_won(buttons[buttonRow][2]['text']))
            return True

    for buttonColumn in range(3):
        if buttons[0][buttonColumn]['text'] == buttons[1][buttonColumn]['text'] == \
                buttons[2][buttonColumn]['text'] != "":
            buttons[0][buttonColumn].config(bg=who_won(buttons[0][buttonColumn]['text']))
            buttons[1][buttonColumn].config(bg=who_won(buttons[1][buttonColumn]['text']))
            buttons[2][buttonColumn].config(bg=who_won(buttons[2][buttonColumn]['text']))
            return True

    if buttons[0][0]['text'] == buttons[1][1]['text'] == buttons[2][2]['text'] != "":
        buttons[0][0].config(bg=who_won(buttons[0][0]['text']))
        buttons[1][1].config(bg=who_won(buttons[1][1]['text']))
        buttons[2][2].config(bg=who_won(buttons[2][2]['text']))
        return True
    elif buttons[0][2]['text'] == buttons[1][1]['text'] == buttons[2][0]['text'] != "":
        buttons[0][2].config(bg=who_won(buttons[0][2]['text']))
        buttons[1][1].config(bg=who_won(buttons[1][1]['text']))
        buttons[2][0].config(bg=who_won(buttons[2][0]['text']))
        return True
...
def player_turn(buttonRow, buttonColumn):
    ...
    randomPlayerR = random.choice(playerTwoR)
    randomPlayerC = random.choice(playerTwoC)
    ...
    if buttons[buttonRow][buttonColumn]['text'] == "" and check_winner() is False:
        ...
        while currentPlayer != players[i]:
            ...
            if check_winner() is False:
                # the random players turn if the user player hasn't won
                while buttons[randomPlayerR][randomPlayerC]['text'] != "":
                    randomPlayerR = random.choice(playerTwoR)
                    randomPlayerC = random.choice(playerTwoC)
                else:
                    buttons[randomPlayerR][randomPlayerC]['text'] = players[i == 0]
...
# check who won the game, color to mark the winning player
def who_won(result):
